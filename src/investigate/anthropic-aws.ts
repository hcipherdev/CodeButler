...
export function createAnthropicAwsInvestigator(
  config: InvestigatorConfig,
  httpClient?: AnthropicAwsHttpClient
): InvestigatorProvider {
  return {
    async planNextAction(state, options) {
      const content = await requestJsonContent(httpClient, config, INVESTIGATOR_SYSTEM_PROMPT, {
    ...
    async synthesizeAnswer(state) {
      const content = await requestJsonContent(httpClient, config, SYNTHESIS_SYSTEM_PROMPT, {
...
async function requestJsonContent(
  httpClient: AnthropicAwsHttpClient | undefined,
  config: InvestigatorConfig,
  systemPrompt: string,
  ...
  return readAnthropicAwsText(response);
}
